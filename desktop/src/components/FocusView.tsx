// The focus view: click a host in the Fleet Strip and its detail pane opens
// below — a real Performance view (larger charts, same data source) plus the
// host's identity readouts. An offline host keeps its layout and shows a
// disconnected plate rather than vanishing.
...
export function FocusView({ host, onRename, onRemove }: FocusProps) {
  const { peer, info, latest, hist, online } = host
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
      ...
      <div className="flex items-center justify-between border-b border-rule px-4 py-2">
        <div className="flex items-baseline gap-3">
          <span className="font-plate text-lg font-semibold uppercase tracking-wider text-readout">
            {peer.name}
          </span>
      ...
      {/* Load line. */}
      {latest && (
        <div className="flex items-center gap-6 border-t border-rule px-4 py-2">
