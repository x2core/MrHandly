...
const SIGNAL = '#E8A33D'
const SIGNAL_FILL = 'rgba(232,163,61,0.16)'
...
function Meter({ label, value, values, max }: { label: string; value: string; values: number[]; max?: number | 'auto' }) {
  return (
    <div className="flex min-w-0 flex-1 flex-col justify-center border-l border-rule px-3">
      ...
      <Sparkline values={values} stroke={SIGNAL} fill={SIGNAL_FILL} max={max ?? 1} />
    </div>
  )
}
...
export function HostStrip({ host, selected, onSelect }: Props) {
  ...
  return (
    ...
    >
      ...
      {online ? (
        <>
          <Meter label="CPU" value={pct(cpu)} values={hist.cpu} />
          <Meter label="RAM" value={latest ? bytes(latest.memory.used) : pct(mem)} values={hist.mem} />
          <Meter label="NET" value={rate(net)} values={hist.net} max="auto" />
        </>
      ) : (
